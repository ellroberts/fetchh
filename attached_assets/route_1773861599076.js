...
function detectPlatformFromData(data) {
  if (data.mapping && typeof data.mapping === 'object') {
    ...
    if (first?.message?.author?.role || first?.message?.content?.parts) return 'chatgpt';
  }
  if (data.uuid || (Array.isArray(data) && data.some(m => m.sender === 'human' || m.sender === 'assistant'))) return 'claude';
  if (Array.isArray(data) && data.length > 0) {
    if (data.some(m => (m.role === 'user' || m.role === 'assistant') && (m.content || m.text) && !m.parts)) return 'claude';
  }
...
function triggerAutoSummary(conversationId, appBaseUrl, authToken) {
  ...
  fetch(url, {
    ...
    .then(res => {
      if (!res.ok) res.text().then(t => console.warn('[save] Auto-summary failed:', res.status, t));
      else console.log('[save] Auto-summary triggered successfully for:', conversationId);
    })
    .catch(err => console.warn('[save] Auto-summary fetch error (non-fatal):', err.message));
}
...
export async function POST(req) {
  ...
  try {
      ...
      try {
        ...
        if (user && !error) {
          ...
          try {
            ...
            authSessionId = sessionData?.session?.access_token ? sessionData.session.access_token.substring(0, 36) : null;
          } catch {}
        } else {
    ...
    const incomingSessionId = (body.session_id && typeof body.session_id === 'string' && body.session_id.length > 0) ? body.session_id : null;
    ...
    if (!userId) {
      if (incomingSessionId) {
        ...
        console.warn('[save] No valid authentication or session_id — rejecting request');
        return NextResponse.json(
          { error: 'Unauthorized', message: 'A valid auth token or session_id is required.' },
          { status: 401, headers: corsHeaders }
        );
      }
    }

    console.log('[save] Auth resolved:', { authMethod, userId, guestSessionId, incomingSessionId, authSessionId });
    ...
    if (body.encrypted_payload && typeof body.encrypted_payload === 'string') {
      if (guestSessionId) {
        console.warn('[save] Guest request sent encrypted_payload — guests must send plaintext conversationData');
        return NextResponse.json(
          { error: 'Guests cannot send encrypted payloads. Send conversationData instead.' },
          { status: 400, headers: corsHeaders }
        );
      }
      isEncrypted = true;
      console.log('[save] encrypted_payload detected (length:', body.encrypted_payload.length, ')');
      ...
      if (userKey) {
        ...
        conversationData = await tryDecrypt(body.encrypted_payload, userKey, encFormat);
      }
      if (!conversationData) {
        console.error('[save] Decryption failed for user', userId, '— per-user key missing or payload corrupt');
        return NextResponse.json(
          { error: 'Decryption failed', message: 'Could not decrypt payload with your encryption key. Please re-login to sync your key.' },
          { status: 400, headers: corsHeaders }
        );
      }
      console.log('[save] Decrypted successfully. Messages count:', conversationData.messages?.length || 0);
    } else if (body.conversationData) {
      conversationData = body.conversationData;
      console.log('[save] Using plaintext conversationData');
    } else {
      console.warn('[save] Body has neither encrypted_payload nor conversationData. Keys:', Object.keys(body));
      return NextResponse.json(
    ...
    let resolvedSource = body.platform || (typeof conversationData === 'object' && !Array.isArray(conversationData) ? conversationData.source : null) || source;
    ...
    if (nativeChatId) console.log('[save] Native chat ID extracted:', nativeChatId);

    const processedData = processConversationData(conversationData, resolvedSource);
    ...
    const resolvedSessionId = guestSessionId || body.sessionId || authSessionId || null;
    ...
    const conversationRecord = {
      ...
      metadata: {
        ...
        imported_via: (authMethod === 'bearer' || authMethod === 'token') ? 'chrome_extension' : (authMethod === 'guest' ? 'chrome_extension_guest' : 'web_interface'),
    ...
    if (userId && (nativeChatId || incomingSessionId)) {
      try {
        ...
        if (guestRow) {
          ...
          if (newCount > existingCount) {
            ...
            console.log('[save] Skipping guest upgrade: new save has more messages (' + newCount + ' vs ' + existingCount + ') — treating as new conversation');
          } else {
            // Same or fewer messages → genuine re-save / claim of the same conversation.
            console.log('[save] Found guest row for upgrade — id:', guestRow.id, 'matched via:', nativeChatId ? 'chat_id' : 'session_id');
            const { error: upgradeError } = await serviceClient
              ...
              .update({
                ...
                metadata: { ...conversationRecord.metadata, upgraded_from_guest: true, upgraded_at: new Date().toISOString() }
              })
              .eq('id', guestRow.id);

            if (!upgradeError) {
              const shareableUrl = `https://threadcub.com/api/share/${guestRow.id}`;
              console.log('[save] Guest row upgraded successfully:', guestRow.id);
              ...
              const summaryToken = req.headers.get('authorization')?.replace('Bearer ', '');

              if (!existingRow?.quick_summary) {
                console.log('[save] quick_summary missing on upgraded row — generating now');
                await Promise.race([
                  fetch(`${appBaseUrl}/api/quick-summary`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json', ...(summaryToken && { 'Authorization': `Bearer ${summaryToken}` }) },
                    body: JSON.stringify({ conversationId: guestRow.id }),
                  }).catch(e => console.warn('[save] Auto-summary error (upgrade):', e.message)),
                  new Promise(resolve => setTimeout(resolve, 4500))
                ]);
              } else {
                console.log('[save] quick_summary already exists on upgraded row — skipping generation');
              }

              // Auto-index if not already embedded
              if (!existingRow?.has_embeddings) {
                console.log('[save] has_embeddings missing on upgraded row — triggering auto-index');
                embedConversation(guestRow.id).catch(err =>
                  console.warn('[save] Auto-index failed (upgrade):', err.message)
                );
              } else {
                console.log('[save] has_embeddings already true on upgraded row — skipping auto-index');
              }
              ...
              console.warn('[save] Guest row upgrade failed (non-fatal), will insert new row:', upgradeError.message);
            }
          }
        }
      } catch (upgradeErr) {
        console.warn('[save] Session→user upgrade check failed (non-fatal):', upgradeErr.message);
      }
    }
    ...
    if (userId) {
      try {
        ...
        const atLimit = !canImportConversation(tier, conversationCount ?? 0)

        if (atLimit) {
          console.log('[save] Tier limit reached for user:', userId, 'tier:', tier, 'count:', conversationCount)
          return NextResponse.json({
            error: 'Conversation limit reached',
            tier,
            conversations_used: conversationCount,
            upgrade_required: true,
            message: `You've reached the conversation limit for your ${tier} plan. Upgrade to save more conversations.`,
          }, { status: 402, headers: corsHeaders })
        }
      } catch (tierErr) {
        console.warn('[save] Tier check failed (non-fatal), allowing save:', tierErr.message)
      }
    }
    ...
    try {
      ...
      if (existingRow) {
        console.log('[save] Duplicate detected within 10s (same title, platform, message_count), skipping insert. Existing id:', existingRow.id);
      ...
      console.warn('[save] Dedup check failed (non-fatal):', dedupErr.message);
    }
    ...
    if (error) {
      if (error.code === '23505') {
        console.warn('[save] Duplicate conversation skipped (23505):', error.details || error.message);
        try {
          ...
          console.warn('[save] 23505 fallback lookup failed:', fallbackErr.message);
        }
        return NextResponse.json({
          success: true,
          message: 'Conversation already saved',
      ...
      console.error('[save] Supabase insert error (full):', JSON.stringify(error, null, 2));
      const isRLS = error.code === '42501' || (error.message && error.message.includes('row-level security'));
      return NextResponse.json({
    ...
    if (!data) {
      console.log('[save] Duplicate conversation skipped (insert returned no row)');
      try {
        const dedupFallback = dbClient
          .from('conversations')
          .select('id')
        ...
        console.warn('[save] !data fallback lookup failed:', fallbackErr.message);
      }
      return NextResponse.json({
        success: true,
        message: 'Conversation already saved',
    ...
    if (userId) {
      ...
      await Promise.race([
        ...
        }).then(r => console.log('[save] Auto-summary status:', r.status)).catch(e => console.warn('[save] Auto-summary error:', e.message)),
        new Promise(resolve => setTimeout(resolve, 4500))
      ]);

      // 7b. Auto-index (embeddings)
    ...
    if (guestSessionId) {
      responseHeaders['Set-Cookie'] = `threadcub_session_id=${guestSessionId}; Path=/; Max-Age=2592000; SameSite=None; Secure`;
    }
    ...
    return NextResponse.json({
      ...
      wasEncrypted: isEncrypted,
      wasDuplicate: false,
      message: userId ? 'Conversation saved to your account' : 'Conversation saved as guest'
...
function processConversationData(rawData, source) {
  ...
  try {
    ...
    switch (source.toLowerCase()) {
      case 'chatgpt': messages = processChatGPTData(rawData); break;
      case 'claude':
      case 'claude.ai': messages = processClaudeData(rawData); break;
      case 'gemini': messages = processGeminiData(rawData); break;
      default: messages = processGenericData(rawData); break;
    }
    messageCount = messages.length;
  } catch (err) {
    console.error('[save] Error processing conversation data:', err);
    messages = [{ role: 'system', content: 'Error processing conversation data. Raw data stored.', timestamp: new Date().toISOString() }];
    messageCount = 1;
  }
  return { messages, messageCount };
}
...
function processGeminiData(data) { return processGenericData(data); }
...
function processGenericData(data) {
  ...
  return [{ role: 'unknown', content: typeof data === 'string' ? data : JSON.stringify(data), timestamp: new Date().toISOString() }];
}

function generateTitle(messages) {
  if (!messages || messages.length === 0) return null;
  const firstUserMessage = messages.find(msg => msg.role === 'user' || msg.role === 'human');
...
function generateSummary(messages) {
  if (!messages || messages.length === 0) return 'No previous conversation context available.';
  ...
  if (userMessages.length > 0) {
    ...
    if (recentMessages.length > 1) summary += ` (${messages.length} total messages exchanged)`;
  } else {
