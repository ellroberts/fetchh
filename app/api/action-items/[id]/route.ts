...
// PATCH  /api/action-items/:id — update status, detail (notes), or tags
// DELETE /api/action-items/:id — delete an action item (Chrome extension)
  ...
  try {
    ...
    if (token) {
      ...
      if (error) {
        ...
        return NextResponse.json({ error: 'Failed to update action item' }, { status: 500, headers: corsHeaders })
      }
...
export async function DELETE(
  ...
  try {
    ...
    if (error) {
      ...
      return NextResponse.json({ error: 'Failed to delete action item' }, { status: 500, headers: corsHeaders })
    }
