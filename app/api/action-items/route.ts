...
// GET  /api/action-items?conversation_id=X  — list action items for a conversation (Chrome extension, Bearer token)
// GET  /api/action-items?project_id=X       — list action items for a project (app internal, cookie auth)
// POST /api/action-items                    — create an action item (Chrome extension)
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (error) {
      console.error('Failed to insert action item:', error)
      return NextResponse.json({ error: 'Failed to save action item' }, { status: 500, headers: corsHeaders })
    }
  ...
  if (token && conversationId) {
    ...
    try {
      ...
      if (error) {
        ...
        return NextResponse.json({ error: 'Failed to fetch action items' }, { status: 500, headers: corsHeaders })
      }
      return NextResponse.json({ action_items: data ?? [] }, { headers: corsHeaders })
    } catch (err: any) {
      return NextResponse.json({ error: err.message }, { status: 500, headers: corsHeaders })
    }
  }
