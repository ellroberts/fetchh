...
import { CodaTrail } from "@/components/rag/CodaTrail";
import { CrumbBlock } from "@/components/rag/CrumbBlock";
...
function RAGFloatingButtonPanel() {
  return (
    <>
      <ComponentTitle
        name="RAGFloatingButton"
        description="Floating Coda bear button that triggers the RAG chat panel. Normally fixed bottom-right."
      />
      <Group title="States">
        <Canvas label="Default — no ready conversations">
        ...
        <Canvas label="Ready — conversations indexed">
          <div style={{ position: "relative", width: "80px", height: "80px" }}>
            <div style={{ position: "absolute", bottom: 0, right: 0 }}>
              <RAGFloatingButton
                onClick={() => {}}
        ...
        <Canvas label="2 conversations selected">
          <div style={{ position: "relative", width: "80px", height: "80px" }}>
            <div style={{ position: "absolute", bottom: 0, right: 0 }}>
              <RAGFloatingButton
                onClick={() => {}}
...
export default function ComponentPlayground() {
  ...
  const panels: Record<string, React.ReactNode> = {
    ...
    RAGFloatingButton: <RAGFloatingButtonPanel />,
    Menu: <MenuPanel />,
    ...
    UserBubble: <UserBubblePanel />,
    CodaBubble: <CodaBubblePanel />,
    CodaTrail: <CodaTrailPanel />,
    CrumbBlock: <CrumbBlockPanel />,
    RagInput: <RagInputPanel />,
    RagChatPanel: <RagChatPanelDemoPanel />,
  };
...
// ── Mock data shared by RAG panels ──────────────────────────────────────────
const MOCK_SOURCES: RagSource[] = [
  {
    conversation_id: "conv-1",
    conversation_title: "Design System Planning",
    chunk_index: 2,
    chunk_text:
      "We decided to use CSS custom properties for all design tokens. This makes it easy to swap themes and ensures consistency across the whole product.",
    similarity: 0.82,
    message_indices: [4, 5, 6],
  },
  {
    conversation_id: "conv-2",
    conversation_title: "Component Library Review",
    chunk_index: 0,
    chunk_text:
      "The button component needs primary, secondary, and ghost variants. We should also consider size variants: sm, md, lg.",
    similarity: 0.61,
    message_indices: [1, 2],
  },
  {
    conversation_id: "conv-3",
    conversation_title: "Sprint Retrospective",
    chunk_index: 5,
    chunk_text:
      "The team identified that component documentation was lacking. We need to add usage examples and prop tables to every component in the library.",
    similarity: 0.43,
    message_indices: [10, 11],
  },
];

function UserBubblePanel() {
  return (
    <div
      style={{
        display: "flex",
    ...
    >
      <ComponentTitle
        name="UserBubble"
        description="Right-aligned chat bubble for user messages. Uses surface-primary-subtle background with text-body colour."
      />
      <SectionCard title="Short message">
        <div style={{ width: "100%" }}>
          <UserBubble content="What are the main themes across my threads?" />
        </div>
      </SectionCard>
      <SectionCard title="Longer message">
        <div style={{ width: "100%" }}>
          <UserBubble content="Can you find any patterns in how I tend to describe problems? I'm curious whether I lean towards technical framing or more conceptual explanations." />
        </div>
      </SectionCard>
      <SectionCard title="With timestamp">
        <div style={{ width: "100%" }}>
          <UserBubble
            content="Summarise what's been discussed in the last sprint retro."
...
function CodaBubblePanel() {
  ...
  return (
    ...
    >
      <ComponentTitle
        name="CodaBubble"
        description="Left-aligned chat bubble for Coda's responses. Uses surface-raised background. Includes an optional pin-on-hover action and a CodaTrail sources section."
      />
      <SectionCard title="Plain response (no sources)">
        <div style={{ width: "100%" }}>
          <CodaBubble content="Your most discussed themes are design systems, API design, and sprint planning — with design systems appearing in nearly 40% of your threads." />
        </div>
      </SectionCard>
      <SectionCard title="With sources + pin on hover">
        <div style={{ width: "100%" }}>
          <CodaBubble
            content="Across your threads, you consistently frame problems in terms of user impact before jumping to solutions. This appears in design discussions as well as engineering trade-off conversations."
      ...
      <SectionCard title="With timestamp">
        <div style={{ width: "100%" }}>
          <CodaBubble
            content="I found 3 relevant threads discussing the design token strategy."
...
function CodaTrailPanel() {
  ...
  return (
    ...
    >
      <ComponentTitle
        name="CodaTrail"
        description="Expandable source citations shown below an assistant message. Paw-print trigger reveals source cards with match quality labels and snippet previews."
      />
      <SectionCard title="Collapsed">
        <CodaTrail
          sources={MOCK_SOURCES}
      ...
      <SectionCard title="Expanded (3 sources)">
        <CodaTrail
          sources={MOCK_SOURCES}
      ...
      <SectionCard title="Single source">
        <CodaTrail
          sources={MOCK_SOURCES.slice(0, 1)}
          isExpanded={true}
          onToggle={() => {}}
...
function CrumbBlockPanel() {
  return (
    ...
    >
      <ComponentTitle
        name="CrumbBlock"
        description="Individual source card inside a CodaTrail. Shows conversation title, a snippet preview, a match quality label, and an optional View in thread action."
      />
      <SectionCard title="Strong match">
        <CrumbBlock source={MOCK_SOURCES[0]} />
      </SectionCard>
      <SectionCard title="Good match">
        <CrumbBlock source={MOCK_SOURCES[1]} />
      </SectionCard>
      <SectionCard title="Partial match">
        <CrumbBlock source={MOCK_SOURCES[2]} />
      </SectionCard>
      <SectionCard title="Loose match — no conversation link">
        <CrumbBlock
          source={{
            conversation_id: "",
            conversation_title: "Quick note (no thread link)",
            chunk_index: 0,
            chunk_text:
              "A loose reference with no conversation to navigate to.",
            similarity: 0.28,
            message_indices: [],
          }}
        />
...
function RagInputPanel() {
  const [value, setValue] = useState("");
  const [loading, setLoading] = useState(false);

  function MockScopeChip({
    label,
    scoped,
  }: {
    label: string;
  ...
  return (
    ...
    >
      <ComponentTitle
        name="RagInput"
        description="Composite chat input. Wraps a growing textarea with a ScopeChip slot on the left and a send/stop button on the right. Built for the RAG chat panel."
      />
      <SectionCard title="Empty (global scope)">
        <div style={{ width: "100%" }}>
          <RagInput
            value=""
            onChange={() => {}}
            ...
            scopeSlot={<MockScopeChip label="All threads" scoped={false} />}
          />
        </div>
      </SectionCard>
      <SectionCard title="Filled (scoped)">
        <div style={{ width: "100%" }}>
          <RagInput
            value="What are the main themes across my threads?"
            onChange={() => {}}
            onSend={() => {}}
            scopeSlot={
              <MockScopeChip label="ThreadCub Rebrand" scoped={true} />
            }
          />
        </div>
      </SectionCard>
      <SectionCard title="Loading (stop button shown)">
        <div style={{ width: "100%" }}>
          <RagInput
            value=""
            onChange={() => {}}
...
function RagChatPanelDemoPanel() {
  const [value, setValue] = useState("");
  const [expanded, setExpanded] = useState(false);
  ...
  const MOCK_TS = new Date(Date.now() - 1000 * 60 * 8).toISOString();
  const MOCK_TS2 = new Date(Date.now() - 1000 * 60 * 5).toISOString();
  const MOCK_TS3 = new Date(Date.now() - 1000 * 60 * 2).toISOString();
  ...
  return (
    ...
    >
      <ComponentTitle
        name="RagChatPanel"
        description="Full panel composition — UserBubble, CodaBubble, CodaTrail, and RagInput working together. This is a static mockup for design reference."
      />
      <SectionCard>
        <div
          style={{
        ...
        >
          ...
          >
            <UserBubble
              content="What are the main themes across my design system threads?"
              timestamp={MOCK_TS}
            />
            <CodaBubble
              content="Your design system threads cluster around three main themes: token architecture, component API consistency, and documentation standards. You've revisited token naming conventions the most — it comes up in about 60% of relevant threads."
              timestamp={MOCK_TS2}
              sources={MOCK_SOURCES}
              isExpanded={expanded}
              onToggleExpand={() => setExpanded((e) => !e)}
            ...
            />
            <UserBubble
              content="Which thread talked the most about documentation?"
              timestamp={MOCK_TS3}
            />
          </div>
          ...
          <div style={{ flexShrink: 0, padding: "16px" }}>
            ...
            >
              Coda can make mistakes. This is a static playground preview.
            </p>
          </div>
        </div>
      </SectionCard>
