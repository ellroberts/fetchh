...
const BASE = '/homepage-test'

const NAV_ITEMS = [
  {
    label: 'Features',
    dropdown: [
      { label: 'Dashboard',  href: `${BASE}/features`, icon: LayoutDashboard, color: 'var(--color-primary-500)' },
      { label: 'Chats',      href: `${BASE}/features`, icon: MessageSquare,    color: 'var(--color-accent-teal)' },
      { label: 'Threads',    href: `${BASE}/features`, icon: MessageSquareText,color: 'var(--color-accent-teal)',  soon: true },
      { label: 'Projects',   href: `${BASE}/features`, icon: FolderOpen,       color: 'var(--color-accent-rose)' },
      { label: 'Storyline',  href: `${BASE}/features`, icon: GitBranch,        color: 'var(--color-accent-purple)', soon: true },
      { label: 'Ask Coda',   href: `${BASE}/features`, icon: Bot,              color: 'var(--color-primary-500)' },
      { label: 'Insights',   href: `${BASE}/features`, icon: Sparkles,         color: 'var(--color-accent-amber)' },
      { label: 'Pawmarks',   href: `${BASE}/features`, icon: PawPrint,         color: 'var(--color-accent-amber)' },
      { label: 'Library',    href: `${BASE}/features`, icon: LibraryBig,       color: 'var(--color-bear-500)',     soon: true },
    ],
  },
  {
    label: 'Use cases',
    dropdown: [
      { label: 'Builders & indie hackers', href: `${BASE}/use-cases/builders`,    icon: Hammer,       color: 'var(--color-accent-rose)' },
      { label: 'Researchers',              href: `${BASE}/use-cases/researchers`, icon: FlaskConical,  color: 'var(--color-accent-teal)' },
      { label: 'Consultants',              href: `${BASE}/use-cases/consultants`, icon: Briefcase,     color: 'var(--color-accent-amber)' },
      { label: 'Lifelong learners',        href: `${BASE}/use-cases/learners`,    icon: BookOpen,      color: 'var(--color-primary-500)' },
    ],
  },
  ...
  return (
    <div style={{ background: 'var(--color-surface-page)', minHeight: '100vh', fontFamily: 'var(--font-family-primary)' }}>
      ...
      <nav style={{
        ...
        {/* Logo */}
        <Link href={BASE} style={{ display: 'flex', alignItems: 'center', gap: 'var(--spacing-3)', textDecoration: 'none' }}>
          <Logo size="sm" />
