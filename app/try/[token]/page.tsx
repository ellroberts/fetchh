...
function resourceMatchesCard(resource: { name: string; url: string }, card?: CarouselCard) {
  ...
  const cardText = [card.title, card.body, ...(card.bullets ?? []), card.url ?? ''].join(' ').toLowerCase()
  ...
  if (cards.length === 0) {
    return (
      <div style={{ display: 'flex', justifyContent: 'center' }}>
        <div style={{ width: CARD_WIDTH, background: '#FFFFFF', borderRadius: CARD_RADIUS, padding: CARD_PADDING, textAlign: 'center', color: '#999', fontSize: 16 }}>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
      <div style={{ position: 'relative', width: '100%' }}>
        <div style={{ maskImage: maskGradient, WebkitMaskImage: maskGradient }}>
              ...
              {Array.from({ length: totalSlides }).map((_, i) => {
                ...
                return (
                  <div
                    ...
                    style={{ flex: `0 0 ${CARD_WIDTH + SLIDE_PADDING_X * 2}px`, paddingLeft: SLIDE_PADDING_X, paddingRight: SLIDE_PADDING_X, display: 'flex' }}
                  >
                    <div
                      style={{
                        flex: 1, background: '#FFFFFF', borderRadius: CARD_RADIUS, minHeight: 400,
        ...
        {canPrev && (
          <button onClick={() => emblaApi?.scrollPrev()} aria-label="Previous" style={{ ...ARROW_STYLE, left: `calc(50% - ${halfCard}px - 32px - 24px)` }}>
            <ArrowLeft size={24} color="#6C74FB" />
          </button>
        )}
        {canNext && (
          <button onClick={() => emblaApi?.scrollNext()} aria-label="Next" style={{ ...ARROW_STYLE, left: `calc(50% + ${halfCard}px + 32px)` }}>
...
export default function TryTokenPage() {
    ...
    <div style={{
      ...
      {channelName && (
        <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexShrink: 0 }}>
          <div style={{ width: 24, height: 24, borderRadius: '50%', background: '#D4D0C8', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 11, fontWeight: 700, color: '#555' }}>
            {channelInitial}
          </div>
  ...
  if (status === 'done' && (result || isMock)) {
    ...
    const worthText = worthField ? normalizeDisplayText(worthField.bodyHtml.replace(/<[^>]+>/g, '').trim()) : ''
        ...
        <>
          ...
          <div style={{ position: 'relative' }} onMouseEnter={() => setHoveringPrompt(true)} onMouseLeave={() => setHoveringPrompt(false)}>
            <textarea
              ...
              rows={6}
              style={{ width: '100%', boxSizing: 'border-box', padding: '14px 44px 14px 14px', border: '1px solid #E5E5E5', borderRadius: 10, fontSize: 15, lineHeight: 1.6, color: '#333', background: '#FAFAFA', resize: 'vertical', fontFamily: 'inherit', outline: 'none' }}
            />
            <div
            ...
            >
              {hoveringCopyBtn && !copied && (
                <div style={{ position: 'absolute', bottom: '100%', right: 0, marginBottom: 6, background: '#111', color: '#FFF', fontSize: 12, fontWeight: 500, padding: '4px 8px', borderRadius: 6, whiteSpace: 'nowrap', pointerEvents: 'none' }}>Copy</div>
              )}
              <button
                onClick={handleCopy}
      ...
      <div style={{ width: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'center', gap: 16, marginBottom: 32 }}>
          {feedbackOptions.map(({ value, label, icon: Icon }) => {
            ...
            return (
              <IconButton key={value} onClick={() => setFeedbackRating(value)} tooltip={label} title={label} size="md" variant="ghost" selected={selected} style={{ color: selected ? '#6C74FB' : '#777', backgroundColor: selected ? '#F0F0FF' : 'transparent', borderRadius: 999 }}>
                <Icon size={22} />
              </IconButton>
            )
          })}
    ...
    const activeContentCard = carouselSelectedIndex < activeCards.length ? activeCards[carouselSelectedIndex] : undefined
    ...
    return (
      <>
        ...
        <main style={PAGE}>
          <a href={youtubeUrl} target="_blank" rel="noopener noreferrer" style={{ display: 'block', width: 330, borderRadius: 16, overflow: 'hidden', flexShrink: 0, marginBottom: 32 }}>
            <img src={thumbnailUrl} alt="Video thumbnail" style={{ display: 'block', width: '100%' }} />
          </a>

          <div style={{ width: '100%', marginBottom: 0 }}>
            <Carousel key={resolvedTab} cards={activeCards} extraForLast={extraContent} endCard={feedbackCard} onSelectedIndexChange={setCarouselSelectedIndex} />
          </div>

          {showQuickLinks && (
            <div style={{ width: CARD_WIDTH, marginTop: 48, textAlign: 'center' }}>
              <p style={{ margin: '0 0 16px', fontSize: 16, fontWeight: 400, color: '#000' }}>Inspired? Here&apos;s some quick links</p>
              <div style={{ display: 'flex', gap: 12, justifyContent: 'center', flexWrap: 'wrap' }}>
                {activeRelevantResources.map((resource) => (
                  <a key={resource.name} href={resource.url} target="_blank" rel="noopener noreferrer" style={{ padding: '8px 12px', borderRadius: 12, border: '1px solid #E5E5E5', background: '#FFFFFF', fontSize: 14, color: '#111111', textDecoration: 'none', cursor: 'pointer' }}>
                    {resource.name}
                  </a>
                ))}
              </div>
          ...
          {worthText && (
            <div style={{ width: CARD_WIDTH, borderRadius: CARD_RADIUS, padding: CARD_PADDING, marginTop: 48 }}>
              <p style={{ margin: '0 0 12px', fontSize: 24, fontWeight: 700, color: '#111111' }}>Worth watching?</p>
              <p style={{ margin: '0 0 16px', fontSize: 16, color: '#333333', lineHeight: 1.6 }}>{worthText}</p>
              <a href={watchUrl} target="_blank" rel="noopener noreferrer" style={{ fontSize: 16, color: '#3086FF', textDecoration: 'none' }} onMouseEnter={(e) => { (e.currentTarget as HTMLAnchorElement).style.textDecoration = 'underline' }} onMouseLeave={(e) => { (e.currentTarget as HTMLAnchorElement).style.textDecoration = 'none' }}>
                Watch now →
              </a>
            </div>
          )}
        ...
        <div style={{ position: 'fixed', bottom: 0, left: 0, right: 0, height: 56, background: '#FFF', borderTop: '1px solid #E5E5E5', zIndex: 100, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 24 }}>
          {visibleTabs.map((tab) => {
            const isActive = tab.key === resolvedTab
            return (
              <button key={tab.key} onClick={() => setActiveTab(tab.key)} style={{ background: isActive ? '#E2E2E2' : 'none', border: 'none', borderRadius: isActive ? 8 : 0, padding: '4px 8px', fontSize: 16, fontFamily: 'Avenir, "Avenir Next", sans-serif', fontWeight: 500, color: '#000', cursor: 'pointer', whiteSpace: 'nowrap' }}>
                {tab.label}
              </button>
            )
          })}
