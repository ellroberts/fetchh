...
interface SelectionMenuPortalHandle {
  show: (data: SelectionMenu) => void;
  hide: () => void;
}

// TODO: RAG save flow — needs proper conversation/project association before re-enabling
/* const SelectionMenuPortal = forwardRef<SelectionMenuPortalHandle, {
  onChoose: (data: SelectionMenu, type: 'insight' | 'action' | 'reminder') => void;
}>(function SelectionMenuPortal({ onChoose }, ref) {
  const [menu, setMenu] = useState<SelectionMenu | null>(null);

  useImperativeHandle(ref, () => ({
    show: (data: SelectionMenu) => setMenu(data),
    hide: () => setMenu(null),
  }));
      ...
      {([
        ...
        <button
          ...
          style={{
            ...
            width: 36, height: 36, borderRadius: '7px', border: 'none',
...
// Project-aware starter prompts with Lucide icons
...
export function RAGChatPanel({ isOpen, onClose, isExpanded, onExpandToggle }: RAGChatPanelProps) {
  ...
  const selectionMenuPortalRef = useRef<SelectionMenuPortalHandle>(null);
  // TODO: RAG save flow — needs proper conversation/project association before re-enabling
  const [ragPendingPin, setRagPendingPin] = useState<{ text: string; ragQuery: string; sourceIds: string[] } | null>(null);
  const [ragPendingType, setRagPendingType] = useState<'insight' | 'action' | 'reminder'>('insight');
  ...
  // On first open, check if we know the user's name yet
  useEffect(() => {
    if (!isOpen) return;
  ...
  useEffect(() => {
    ...
    const loadPending = async () => {
      const { data } = await supabase
        ...
        .single()
      if (data) {
        handleLoadSession(data as RagSession)
      }
      clearPendingSession()
    }
    loadPending()
  ...
  const handleSend = async (overrideValue?: string) => {
    ...
    // ── Name capture: intercept the very first reply ──────────────────────
    if (isAwaitingName) {
      ...
      const { data: { user } } = await supabase.auth.getUser();
      if (user) {
        supabase.from('user_profiles').update({ display_name: name }).eq('id', user.id).then(() => {});
      }
      return;
    }
    // ─────────────────────────────────────────────────────────────────────

    // !! DO NOT REMOVE — triggers onboarding "tried_rag" step completion !!
    window.dispatchEvent(new CustomEvent('threadcub:rag-queried'));
    setInputValue('');
    setError(null);
    setIsExiting(true);
    ...
    try {
      const response = await fetch('/api/chat/multi-analyze', {
        ...
        body: JSON.stringify({ conversation_ids: libraryDocIds ? [] : (globalMode ? [] : scopedConversationIds), library_doc_ids: libraryDocIds ?? undefined, question, session_id: sessionId, project_name: globalMode ? null : (projectName ?? null), project_id: globalMode ? null : (projectId ?? null) }),
      ...
      const assistantMessage: RagChatMessage = { role: 'assistant', content: data.answer, timestamp: new Date().toISOString(), sources: data.sources, caveat: data.caveat ?? null, action_item_added: addedCounts };

      const updatedMessages = [...messagesWithUser, assistantMessage];
      setMessages(updatedMessages);

      if (addedCounts.actions > 0 && projectId) {
        window.dispatchEvent(new CustomEvent('threadcub:action-item-added', { detail: { projectId } }));
      }
      if (addedCounts.reminders > 0 && projectId) {
        window.dispatchEvent(new CustomEvent('threadcub:reminder-item-added', { detail: { projectId } }));
      }
                          ...
                          <>
                            {hasProject && (
                              <div style={{ fontSize: "var(--font-size-xs)", fontWeight: "var(--font-weight-medium)", marginBottom: "3px", display: "flex", alignItems: "center", gap: "4px" }}>
                                <span style={{ color: projectRemoved ? "var(--color-text-muted)" : "var(--color-primary-500)" }}>
                                  {session.project_name}
                                </span>
                                {projectRemoved && (
                                  <span style={{ color: "var(--color-text-muted)", fontStyle: "italic", fontWeight: "var(--font-weight-regular)" }}>(removed)</span>
                                )}
                              </div>
                            )}
                            <div style={{ fontSize: "var(--font-size-sm)", fontWeight: "var(--font-weight-medium)", color: "var(--color-text-title)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", marginBottom: "4px" }}>
                              {session.title}
                            </div>
  ...
  return (
    <div className="h-full w-full flex flex-col" style={{ backgroundColor: "var(--color-surface-raised)", borderRadius: "4px" }}>
      ...
      {/* Messages */}
      {/* TODO: RAG save flow — needs proper conversation/project association before re-enabling */}
      {/* <SelectionMenuPortal ref={selectionMenuPortalRef} onChoose={handleRagChoose} /> */}
      <div
      ...
      >
        ...
        {messages.length === 0 && (
          <div key={isOpen ? 'open' : 'closed'} className="flex flex-col h-full">
            <style>{`
              ...
              .animate-exit {
                animation: fadeSlideDown 0.25s ease-in both;
              }
            `}</style>
        ...
        {messages.map((msg, index) =>
          msg.role === 'user' ? (
            ...
            <div key={index} style={{ display: 'flex', flexDirection: 'column', gap: 'var(--spacing-2)' }}>
              {msg.caveat && (
                <Alert type="warning" size="md" dismissible={false}>
                  {msg.caveat}
                </Alert>
              )}
      ...
      {/* TODO: RAG save flow — needs proper conversation/project association before re-enabling */}
      {/* Text selection modals — rendered via portal so they cover full app */}
      {/* {ragPendingPin && typeof document !== 'undefined' && createPortal(
        <>
          ...
        </>,
        document.body
      ...
      <div className="flex-shrink-0 p-4">
        ...
        <p style={{ fontSize: "var(--font-size-xs)", color: "var(--color-text-muted)", opacity: 0.7, marginTop: "8px", textAlign: "center" }}>
          ...
          <a href="/help" style={{ color: "var(--color-text-muted)", textDecoration: "underline" }}>
            here
          </a>
        </p>
      </div>
