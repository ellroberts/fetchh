...
export interface HighlightRowCardProps {
  id: string
  content: string
  note?: string
  tag?: string
...
export function HighlightRowCard({
  content,
  note,
  tag,
  sourceTitle,
  ...
  return (
    <div className={['jump-row-card', className].filter(Boolean).join(' ')}>
      <div className="jump-row-card__body" onClick={onViewDetails}>
        ...
        {note && <p style={{ fontStyle: 'italic', fontSize: 'var(--font-size-sm)', color: 'var(--color-text-secondary)', margin: '4px 0 0' }}>{note}</p>}
        <div className="jump-row-card__meta">
          <span suppressHydrationWarning className="jump-row-card__meta-text">{date}</span>
          {sourceTitle && (
            <>
              <StatDot />
              <span className="jump-row-card__meta-text">From {sourceTitle}</span>
            </>
          )}
        </div>
      </div>
...
export default HighlightRowCard
